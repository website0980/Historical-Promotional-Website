...
$id = $_GET['edit'] ?? $_GET['id'] ?? $_POST['id'] ?? null;
if ($id !== null && $id !== '') {
    $id = (int) $id;
}
$errors = [];
...
if ($id !== null && $id !== '' && isset($naturalWonders[$id])) {
    $isEdit = true;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        if (isset($_FILES['image_file']) && $_FILES['image_file']['error'] === UPLOAD_ERR_OK) {
            $result = saveNaturalWonderImage($_FILES['image_file']);
            if ($result['success']) {
                if (!empty($wonder['image'])) {
                    deleteNaturalWonderImage($wonder['image']);
                }
                $wonder['image'] = $result['path'];
            } else {
